...
#response code was set to 302 because, log in is required to access each view, so therefore, it is going to be forbidden
    def test_view_url_exists_at_desired_location(self):
        response = self.client.get('/catalog/author/')
        self.assertEqual(response.status_code, 200)

    def test_view_url_accessible_by_name(self):
        response = self.client.get(reverse('catalog:author'))
        self.assertEqual(response.status_code, 200)
...
class RenewBookInstancesViewTest(TestCase):
    ...
    def test_redirects_to_all_borrowed_book_list_on_success(self):
        ...
        self.assertRedirects(response, reverse('catlog:book'))
